...
// In-memory store VPS links (reset khi server restart)
const vpsLinks = [];
      ...
      - name: Auto Restart
        ...
        run: |
          curl -X POST \`
            ...
            "https://api.github.com/repos/\$env:GITHUB_REPOSITORY/actions/workflows/vps-windows.yml/dispatches" \`
            -d '{"ref":"main"}'
        env:
      ...
      - name: Install Desktop
        run: |
          ...
          sudo DEBIAN_FRONTEND=noninteractive apt-get install -y \\
            ...
            novnc websockify \\
            openjdk-21-jdk wget curl dbus-x11 \\
            --no-install-recommends -qq

${isMC ? `      - name: Download TLauncher
        run: |
          mkdir -p ~/Desktop
          wget -q "https://tlauncher.org/jar" -O ~/Desktop/TLauncher.jar
          chmod +x ~/Desktop/TLauncher.jar
...
app.post('/api/create', async (req, res) => {
  ...
  try {
    ...
    if (!dispatched) return res.status(400).json({ error: 'Dispatch thất bại. Repo cần có branch main/master.' });
